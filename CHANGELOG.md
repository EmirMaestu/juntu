...
### Added
- **CI/CD + infra (en construcción).** Deploy automatizado por GitHub Actions: PRs con checks (build del frontend + tests del backend), `main` protegida, deploy a producción al crear una **release** (con backup de DB, health-check y **rollback** automático), versionado por releases, y monitoreo con **alerta de caída por Telegram (solo al owner)**. El repo pasa a ser el único source of truth y el VPS se actualiza desde git (se reconcilió `vps_current/` como espejo completo del backend, +44 archivos reales que faltaban, y se limpiaron los `.bak`). Scripts en `deploy/` (`deploy.sh`, `check.sh`, sudoers, units) y workflows en `.github/workflows/`. Endpoint `GET /api/health`. Spec/plan en `docs/superpowers/`.
...
- **"Tu día" clickeable.** Cada item del inicio ahora lleva a su sección (evento/recordatorio → Agenda, tarea → Tareas, recurrente → Recurrentes).
